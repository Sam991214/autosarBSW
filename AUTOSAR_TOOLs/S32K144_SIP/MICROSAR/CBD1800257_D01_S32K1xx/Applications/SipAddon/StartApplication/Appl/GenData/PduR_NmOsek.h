...
#if !defined (PDUR_NMOSEK_H)
# define PDUR_NMOSEK_H
 ...
 *********************************************************************************************************************/
/*!
 * \brief    The function is called by the NmOsek to indicate the complete reception of a NmOsek I-PDU.\n
 *           The PDU Router evaluates the NmOsek I-PDU handle and identifies the destination(s) of the PDU.\n
 *           The call is routed to an upper IF module using the appropriate I-PDU handle of the destination layer.
 * \param    RxPduId           ID of the received NmOsek I-PDU
 ...
 *           PduR_NmOsekRxIndication calls for the same NmOsekRxPduId.
 * \trace    DSGN-PduR_IF_Forwarding
 * \trace    SPEC-666, SPEC-674, SPEC-1096, SPEC-2020085
 * \note     The function is called by NmOsek.
 *********************************************************************************************************************/
FUNC(void, PDUR_CODE) PduR_NmOsekRxIndication(PduIdType RxPduId, P2CONST(PduInfoType, AUTOMATIC, PDUR_APPL_DATA) info);

/* PduR_NmOsekTxConfirmation API disabled due to disabled TxConfirmation in PduRBswModule settings. */

/**********************************************************************************************************************
 * PduR_NmOsekTriggerTransmit
 *********************************************************************************************************************/
/*!
 * \brief    The function is called by the NmOsek to request the NmOsek TX I-PDU before transmission.\n
 *           The PDU Router evaluates the NmOsek I-PDU handle and identifies the destination(s) of the PDU.
 *           The call is routed to an upper IF module using the appropriate I-PDU handle of the destination layer.
 * \param          TxPduId       ID of the NmOsek I-PDU that will be transmitted
 ...
 * \pre      PduR_Init() is executed successfully.
 * \context  This function can be called on interrupt and task level and has not to be interrupted by other\n
 *           PduR_NmOsekTriggerTransmit calls for the same NmOsekTxPduId.
 * \trace    DSGN-PduR_IF_Forwarding
